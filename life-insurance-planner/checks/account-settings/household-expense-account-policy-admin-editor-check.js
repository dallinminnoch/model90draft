...
assert.doesNotMatch(
  editorSource,
  /data-state-cost-adjustment|data-bucket-state-adjustment-multiplier-input|stateAdjustmentEnabled/
);
assert.doesNotMatch(
  editorSource,
...
assert.equal(Object.prototype.hasOwnProperty.call(editor, "STATE_CODE_VALUES"), false, "state multiplier editor API should be retired");
...
assert.equal(Object.prototype.hasOwnProperty.call(missingModel, "stateCostAdjustmentMultipliers"), false, "editor model should not include retired state multiplier controls");
...
assert.equal(
  missingModel.model90DefaultBucketFloors.rows.every((row) => Object.prototype.hasOwnProperty.call(row, "stateAdjustmentEnabled") === false),
  true,
  "MODEL90 default floor rows should not expose retired state adjustment flags"
);
assert.equal(
...
assert.match(missingHtml, /data-household-expense-compressed-controls-summary/);
assert.match(missingHtml, /Compressed Expenses Controls/);
assert.doesNotMatch(missingHtml, /41 graph-affecting rows/, "compressed controls summary should show only the title");
assert.match(missingHtml, /data-household-expense-lifestyle-range-editor-list/);
...
assert.doesNotMatch(missingHtml, /data-household-expense-lifestyle-range-editor-table/, "graph controls should no longer render as a wide fixed table");
assert.equal(
  (missingHtml.match(/admin-household-expense-policy-row-card/g) || []).length,
  defaultSliderEligibleRows.length,
  "each slider-eligible graph row should render as one readable collapsible row card"
);
assert.equal(
  (missingHtml.match(/<details class="admin-household-expense-policy-row-card"/g) || []).length,
  defaultSliderEligibleRows.length,
  "each slider-eligible graph row should render as a native details card"
);
assert.equal(
  (missingHtml.match(/<summary class="admin-household-expense-policy-row-summary"/g) || []).length,
  defaultSliderEligibleRows.length,
  "each slider-eligible graph row should render a visible summary"
);
assert.doesNotMatch(missingHtml, /admin-household-expense-policy-summary-metric/, "collapsed row summaries should show only the expense title");
assert.doesNotMatch(
  missingHtml,
  /<details class="admin-household-expense-policy-control-group"[^>]*\sopen\b|<details class="admin-household-expense-policy-row-card"[^>]*\sopen\b/,
  "compressed controls and individual graph cards should be collapsed by default"
);
...
assert.equal(
  (missingHtml.match(/data-household-expense-policy-reset-row/g) || []).length,
  defaultSliderEligibleRows.length,
  "each slider-eligible graph row should render one ratio reset control"
);
assert.equal(
...
assert.equal(
  (missingHtml.match(/data-state-cost-adjustment/g) || []).length,
  0,
  "retired State Cost editor controls should not render"
);
assert.equal(
...
assert.doesNotMatch(
  missingHtml,
  /data-state-cost-adjustment|data-bucket-state-adjustment-multiplier-input/,
...
assert.equal(Object.prototype.hasOwnProperty.call(foodAtHomePayload.accountPolicy.livingFloorAssumptions, "stateCostAdjustmentMultipliers"), false, "Food at Home save should drop retired state multiplier assumptions");
assert.deepEqual(
...
assert.equal(Object.prototype.hasOwnProperty.call(foodAtHomeResetPayload.accountPolicy.livingFloorAssumptions, "stateCostAdjustmentMultipliers"), false, "Food at Home reset should drop retired state multiplier assumptions");
assert.deepEqual(
...
assert.equal(Object.prototype.hasOwnProperty.call(model90DefaultFloorsPayload.accountPolicy.livingFloorAssumptions.model90DefaultBucketFloors.communicationsConnectivity, "stateAdjustmentEnabled"), false, "stateAdjustmentEnabled should not save");
...
assert.equal(Object.prototype.hasOwnProperty.call(model90DefaultFloorsPayload.accountPolicy.livingFloorAssumptions, "stateCostAdjustmentMultipliers"), false, "MODEL90 default floor save should drop retired state multipliers");
...
assert.equal(Object.prototype.hasOwnProperty.call(model90DefaultFloorsResetPayload.accountPolicy.livingFloorAssumptions.model90DefaultBucketFloors.transportationBasics, "stateAdjustmentEnabled"), false, "MODEL90 default floor reset should not restore retired state adjustment flag");
...
assert.equal(Object.prototype.hasOwnProperty.call(model90DefaultFloorsResetPayload.accountPolicy.livingFloorAssumptions, "stateCostAdjustmentMultipliers"), false, "MODEL90 default floor reset should drop retired state multipliers");
