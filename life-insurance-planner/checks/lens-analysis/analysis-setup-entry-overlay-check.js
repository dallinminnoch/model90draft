...
assert.match(componentsCss, /\.lens-assumptions-overlay[\s\S]*background: color-mix\(in srgb, var\(--m90-text-primary\) 28%, transparent\)/);
...
assert.match(componentsCss, /\.analysis-setup-panel-body,\s*[\s\S]*\.analysis-setup-control-group--policy-returns\s*\{[\s\S]*scrollbar-color: var\(--m90-text-muted\) var\(--m90-surface-secondary\)/);
