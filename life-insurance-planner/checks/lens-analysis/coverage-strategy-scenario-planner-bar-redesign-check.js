...
assert.match(componentsSource, /\.coverage-strategy-projected-dependent-list\s*\{[\s\S]*max-height: 4\.75rem/);
assert.match(componentsSource, /\.coverage-strategy-projected-dependent-row\s*\{[\s\S]*grid-template-columns: minmax\(6rem, 1fr\) 4\.4rem 3rem/);
assert.match(componentsSource, /\.coverage-strategy-diagnostic-export-button\s*\{/);
assert.match(componentsSource, /padding-bottom: clamp\(7rem, 14vh, 8\.5rem\)/);
