...
);
{
  ...
  const transitionGraphModel = attachStableLayoutFrame(makeGraphModel(), {
    zeroCrossingAnchorScenarioId: "transition-selected",
    zeroCrossingAnchorMonth: 30,
    zeroCrossingAnchorSource: "current-rendered-scenario-depletion",
    xDomainMonths: 60,
    yDomain: {
      min: -125000,
  ...
  transitionGraphModel.transitionPeriod = {
    lengthMonths: 6,
  ...
  transitionGraphModel.trace.selectedScenarioId = "transition-selected";
  transitionGraphModel.series.appliedRunwayScenarios = [
    {
      scenarioId: "transition-selected",
      selected: true,
      label: "Transition projection",
      pathId: "postDeathResources",
      pathMode: "linear",
      survivorResourcesAtDeathPoint: { date: "2031-04-29", monthIndex: 0, visualMonthIndex: 0, value: 720000, xRatio: 0.42, yRatio: 0.02 },
      transitionBridgePoints: [
        { date: "2031-04-29", monthIndex: 0, visualMonthIndex: 0, value: 720000, xRatio: 0.42, yRatio: 0.02, transitionBridge: true },
        { date: "2031-10-29", monthIndex: 0, visualMonthIndex: 6, value: 720000, xRatio: 0.5, yRatio: 0.02, transitionBridge: true }
      ],
      fundedRunwayPoints: [
        { date: "2031-04-29", monthIndex: 0, visualMonthIndex: 0, value: 720000, xRatio: 0.42, yRatio: 0.02, transitionBridge: true },
        { date: "2031-10-29", monthIndex: 0, visualMonthIndex: 6, value: 720000, xRatio: 0.5, yRatio: 0.02, transitionBridge: true },
        { date: "2031-11-29", monthIndex: 1, rawMonthIndex: 1, visualMonthIndex: 7, value: 700000, xRatio: 0.51, yRatio: 0.04 },
        { date: "2033-04-29", monthIndex: 24, rawMonthIndex: 24, visualMonthIndex: 30, value: 0, xRatio: 0.84, yRatio: 0.48 }
      ],
      depletionPoint: { date: "2033-04-29", monthIndex: 24, rawMonthIndex: 24, visualMonthIndex: 30, value: 0, xRatio: 0.84, yRatio: 0.48 },
      deficitPoints: [
        { date: "2033-04-29", monthIndex: 24, rawMonthIndex: 24, visualMonthIndex: 30, value: 0, xRatio: 0.84, yRatio: 0.48 },
        { date: "2033-10-29", monthIndex: 30, rawMonthIndex: 30, visualMonthIndex: 36, value: -100000, xRatio: 0.9, yRatio: 0.85, deficitVisualClipped: true }
      ],
      trace: {
        renderSource: "fundedRunwayPoints",
        transitionBridgePointCount: 2
      }
    }
  ];
  const transitionFixture = JSON.parse(JSON.stringify(fixture));
  transitionFixture.graphModel = transitionGraphModel;
  transitionFixture.financialStoryline = {
    graphDotCandidates: [
      {
  ...
  assert.match(
    ...
    "Transition bridge should render flat after death before the shifted raw runway point."
  );
  assert.equal(
    getSvgNumericAttribute(getSvgTag(transitionHtml, "line", "data-income-impact-graph-death-axis"), "x1"),
    185,
    "Death marker should remain fixed while the transition bridge renders."
  );
  assert.equal(
    getSvgNumericAttribute(getSvgTag(transitionHtml, "line", "data-income-impact-graph-zero-baseline"), "y1"),
    265,
    "Zero line should remain fixed while the transition bridge renders."
  );
  const transitionDepletionPosition = getTranslateCoordinates(getRunwayDepletionMarkerTag(transitionHtml, "transition-selected"));
  assert.equal(transitionDepletionPosition.x, 781, "Depletion marker should use shifted visual depletion month.");
  const transitionDeficitPath = getPathD(transitionHtml, "data-income-impact-graph-deficit-area", "postDeathDeficitArea--selected");
  assert.match(
    transitionDeficitPath,
    /^M781 265 L781 265/,
    "Required support area should begin at the shifted visual depletion location."
  );
  const shiftedStorylineDot = getSvgGroupTagByAttribute(transitionHtml, "data-income-impact-storyline-event-id", "cash-savings-depleted");
  const shiftedStorylinePosition = getTranslateCoordinates(shiftedStorylineDot);
  assert.equal(shiftedStorylinePosition.x, 781, "Post-death storyline dots should shift with the visual transition mapping.");
  assert.match(shiftedStorylineDot, /data-income-impact-storyline-month-offset="30"/);
  assert.match(transitionHtml, /data-income-impact-death-conversion-diamond[\s\S]*data-income-impact-storyline-event-id="death-income-stops"/);
  assert.doesNotMatch(transitionHtml, /<g\b(?=[^>]*data-income-impact-storyline-dot(?:\s|>))(?=[^>]*data-income-impact-storyline-event-id="death-income-stops")/);

  const zeroTransitionGraphModel = attachStableLayoutFrame(JSON.parse(JSON.stringify(transitionGraphModel)), {
    zeroCrossingAnchorMonth: 24,
    xDomainMonths: 60
  });
  zeroTransitionGraphModel.transitionPeriod.lengthMonths = 0;
  zeroTransitionGraphModel.series.appliedRunwayScenarios[0].fundedRunwayPoints = [
    { date: "2031-04-29", monthIndex: 0, visualMonthIndex: 0, value: 720000, xRatio: 0.42, yRatio: 0.02 },
    { date: "2031-05-29", monthIndex: 1, rawMonthIndex: 1, visualMonthIndex: 1, value: 700000, xRatio: 0.43, yRatio: 0.04 },
    { date: "2033-04-29", monthIndex: 24, rawMonthIndex: 24, visualMonthIndex: 24, value: 0, xRatio: 0.84, yRatio: 0.48 }
  ];
  zeroTransitionGraphModel.series.appliedRunwayScenarios[0].depletionPoint = { date: "2033-04-29", monthIndex: 24, rawMonthIndex: 24, visualMonthIndex: 24, value: 0, xRatio: 0.84, yRatio: 0.48 };
  const zeroTransitionHtml = harness.renderTimeline({
    ...fixture,
  ...
  assert.match(
    zeroTransitionPath,
    /^M185 36 L209 42 L781 265$/,
    "0-month transition should preserve the unshifted visual runway coordinates."
  );
}
