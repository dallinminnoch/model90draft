(function (window) {
  ...
  function projectGraphPoint(graphModel, point = {}) {
    const safePoint = isPlainObject(point) ? point : {};
    const xRatio = toOptionalNumber(safePoint.xRatio);
    const yRatio = toOptionalNumber(safePoint.yRatio);
    return {
      x: xRatio == null ? null : toGraphX(xRatio, graphModel, safePoint),
      y: yRatio == null ? null : toGraphY(yRatio, graphModel, safePoint),
      xRatio,
      yRatio,
      projectionOwner: getStableGraphLayoutFrame(graphModel) ? "active-view-frame" : "raw-graph-ratio"
    };
  }

  function projectGraphMonthX(graphModel, monthIndex) {
    const month = toOptionalNumber(monthIndex);
    const xRatio = resolveGraphMonthXRatio(graphModel, month);
    return projectGraphPoint(graphModel, {
      xRatio,
      monthIndex: month,
      relativeMonthsFromDeath: month
    }).x;
  }
  ...
  function getDeathLineAnchorLabelPosition(anchor, index, graphModel = null) {
    const position = projectGraphPoint(graphModel, anchor);
    const x = position.x;
    ...
    return `
      <g class="income-impact-graph-markers" data-income-impact-graph-markers>
        ${markers.map(function (marker) {
          const position = projectGraphPoint(graphModel, marker);
          const x = position.x;
          const y = position.y;
  ...
  function getGraphStorylineEventDots(timelineResult, graphModel) {
    ...
    return candidates.slice(0, GRAPH_STORYLINE_EVENT_DOT_LIMIT).map(function (candidate, index) {
      ...
      const position = projectGraphPoint(graphModel, trendlineCoordinate);
      return {
        candidate,
        index,
        x: position.x,
  ...
  function getDeathStorylineMarkerTarget(candidate, graphModel) {
    ...
    const startProjection = projectGraphPoint(graphModel, {
      xRatio: connector.xRatio,
      yRatio: connector.startYRatio,
      value: connector.startValue,
      phase: "deathEvent"
    });
    const endProjection = projectGraphPoint(graphModel, {
      xRatio: connector.xRatio,
      yRatio: connector.endYRatio,
      value: connector.endValue,
      phase: "deathEvent"
    });
    return {
      x: startProjection.x,
      y: Math.min(startProjection.y, endProjection.y),
  ...
  function getSelectedDeficitLabelPosition(selectedSeries, zeroYRatio, graphModel = null) {
    ...
    const position = projectGraphPoint(graphModel, {
      xRatio: anchorXRatio,
      yRatio: anchorYRatio,
      value: getSeriesPointValue(depletionPoint || firstDeficitPoint),
      monthIndex: toOptionalNumber((depletionPoint || firstDeficitPoint)?.monthIndex),
      relativeMonthsFromDeath: toOptionalNumber((depletionPoint || firstDeficitPoint)?.relativeMonthsFromDeath)
    });
    return {
      x: clampNumber(position.x + 12, minX, maxX),
      y: clampNumber(position.y + 24, minY, maxY)
    };
  }
  ...
  function getAppliedScenarioDepletionMarkers(graphModel, selectedScenarioId = "") {
    ...
    return markerSeries.map(function (series) {
      ...
      return {
        ...
        monthIndex: toOptionalNumber(depletionPoint.monthIndex),
        relativeMonthsFromDeath: toOptionalNumber(depletionPoint.relativeMonthsFromDeath),
        relativeYearsFromDeath: toOptionalNumber(depletionPoint.relativeYearsFromDeath),
        phase: normalizeString(depletionPoint.phase),
  ...
  function renderDeathEventConversionConnector(graphModel, timelineResult = null) {
    ...
    const y1 = startProjection.y;
    const y2 = endProjection.y;
    ...
    return `
      ...
      <g
        ...
        data-income-impact-death-conversion-projection-owner="${escapeHtml(startProjection.projectionOwner)}"
  ...
  function renderGraphDeathAnchor(graphModel) {
    ...
    const position = projectGraphPoint(graphModel, anchor);
    return `
      <g class="income-impact-graph-current-anchor" data-income-impact-graph-current-anchor transform="translate(${position.x} ${position.y})">
  ...
  function renderPostDeathFocusRunwayStartMarker(graphModel) {
    ...
    const position = projectGraphPoint(graphModel, startPoint);
    const x = position.x;
    const y = position.y;
    return `
      <g
  ...
  function resolveGraphMonthX(graphModel, monthIndex) {
    return projectGraphMonthX(graphModel, monthIndex);
  }
  ...
  function buildMilestoneDotCandidate(dot, step, tier, index) {
    ...
    return {
      ...
      visibleEventKey: normalizeString(dot?.visibleEventKey || step?.visibleEventKey || dot?.trace?.visibleEventKey || step?.trace?.visibleEventKey),
      cardConceptId: normalizeString(dot?.cardConceptId || step?.cardConceptId || dot?.trace?.cardConceptId || step?.trace?.cardConceptId),
      conceptId: normalizeString(dot?.conceptId || step?.conceptId || dot?.trace?.conceptId || step?.trace?.conceptId),
      storyStage: normalizeString(dot?.storyStage || step?.storyStage || dot?.trace?.storyStage || step?.trace?.storyStage),
      bucketFamily: normalizeString(dot?.bucketFamily || step?.bucketFamily || dot?.trace?.bucketFamily || step?.trace?.bucketFamily),
      bucketId: normalizeString(dot?.bucketId || step?.bucketId || dot?.trace?.bucketId || step?.trace?.bucketId),
      eventState: normalizeString(dot?.eventState || step?.eventState || dot?.trace?.eventState || step?.trace?.eventState),
      stateRank: toOptionalNumber(dot?.stateRank ?? step?.stateRank ?? dot?.trace?.stateRank ?? step?.trace?.stateRank),
      ...
      trace: Object.assign({}, isPlainObject(dot?.trace) ? dot.trace : {}, {
        ...
        visibleEventKey: normalizeString(dot?.visibleEventKey || step?.visibleEventKey || dot?.trace?.visibleEventKey || step?.trace?.visibleEventKey) || null,
        cardConceptId: normalizeString(dot?.cardConceptId || step?.cardConceptId || dot?.trace?.cardConceptId || step?.trace?.cardConceptId) || null,
        conceptId: normalizeString(dot?.conceptId || step?.conceptId || dot?.trace?.conceptId || step?.trace?.conceptId) || null,
        storyStage: normalizeString(dot?.storyStage || step?.storyStage || dot?.trace?.storyStage || step?.trace?.storyStage) || null,
        bucketFamily: normalizeString(dot?.bucketFamily || step?.bucketFamily || dot?.trace?.bucketFamily || step?.trace?.bucketFamily) || null,
        bucketId: normalizeString(dot?.bucketId || step?.bucketId || dot?.trace?.bucketId || step?.trace?.bucketId) || null,
        eventState: normalizeString(dot?.eventState || step?.eventState || dot?.trace?.eventState || step?.trace?.eventState) || null,
        stateRank: toOptionalNumber(dot?.stateRank ?? step?.stateRank ?? dot?.trace?.stateRank ?? step?.trace?.stateRank),
